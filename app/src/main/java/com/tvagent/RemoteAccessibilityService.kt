...
class RemoteAccessibilityService : AccessibilityService() {
    ...
    companion object {
        @Volatile var lastStatus: String = "Service not started yet"
    }
    ...
    override fun onServiceConnected() {
        ...
        lastStatus = "Service started, connecting..."
        ...
        try {
            ...
            client.setCallback(object : MqttCallback {
                override fun connectionLost(cause: Throwable?) {
                    lastStatus = "Connection lost: ${cause?.message}"
                    Log.w("TVAgent", lastStatus)
                }

                override fun messageArrived(t: String?, message: MqttMessage?) {
                    val command = message?.toString()?.trim() ?: return
                    lastStatus = "Last command received: $command"
            ...
            mqttClient = client
            lastStatus = "Connected, listening for commands"
            Log.i("TVAgent", lastStatus)
        } catch (e: Exception) {
            lastStatus = "Connect FAILED: ${e.javaClass.simpleName} - ${e.message}"
            Log.e("TVAgent", lastStatus, e)
        }
    }
